...
class _BusinessPetsDetailsScreenState extends State<BusinessPetsDetailsScreen>
  ...
  // Keep exact listener references to remove in dispose
  late final void Function(int) _completedPageListener;
  late final void Function(int) _pendingPageListener;

  @override
  void initState() {
    super.initState();

    // Define listeners only once per State instance
    _completedPageListener = (int pageKey) {
    ...
    _pendingPageListener = (int pageKey) {
    ...
    businessAllPetController.pagingController.addPageRequestListener(_completedPageListener);
    businessAllPetController.pagingController1.addPageRequestListener(_pendingPageListener);

    // Defer any Rx mutations (which cause Obx rebuilds) until after the first frame
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;

      // Load header/details safely after first build
      businessAllPetController.businessPetDetails(id: widget.id);

      // If you want an initial page primed immediately:
      businessAllPetController.getHealthHistoryUpdate(
        id: widget.id,
        status: '',
        page: 1,
      ...
      // Let paging controllers request first pages (triggers listeners)
  ...
  @override
  void dispose() {
    // Clean up listeners to avoid leaks / duplicate calls on hot-rebuilds or nav back/forward
    businessAllPetController.pagingController.removePageRequestListener(_completedPageListener);
    businessAllPetController.pagingController1.removePageRequestListener(_pendingPageListener);
    super.dispose();
  }
      ...
      child: Scaffold(
        ...
        body: NestedScrollView(
          headerSliverBuilder: (context, innerScrolled) => [
            ...
            SliverToBoxAdapter(
              child: Obx(() {
                final petPhoto = businessAllPetController.details.value.pet?.petPhoto;
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const Gap(6),
                        Obx(() {
                          final name = businessAllPetController.details.value.pet?.name ?? "";
                          return CustomText(
                            text: "About $name",
                            fontWeight: FontWeight.w600,
                            fontSize: 16,
                    ...
                    Row(
                      children: const [
                        ...
                        Gap(6),
                        CustomText(text: "Status", fontWeight: FontWeight.w600, fontSize: 16),
                      ],
                    ),
                    const Gap(16),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: () => showAddHealthDialog(context, widget.id),
                          child: Container(
