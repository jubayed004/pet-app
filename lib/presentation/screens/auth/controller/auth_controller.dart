...
class AuthController extends GetxController {
      ...
      if (response.statusCode == 200) {
        ...
        dbHelper
            .saveUserdata(
              ...
              role: role,
              rememberMe: true, // Hardcoded to true for persistent session
            )
            .then((value) {
              loginMethod(false);
              if (role == "USER") {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        AppRouter.route.pushNamed(
          RoutePath.accountActiveOtpScreen,
          extra: body,
        );
        nameSignUp.clear();
        emailSignUp.clear();
  ...
  shopRegistrationUpLoadingMethod(bool status) =>
      shopRegistrationUpLoading.value = status;
  ...
  Future<void> petShopRegistration() async {
    try {
      ...
      if (selectedLogo.value != null) {
        multipartBody.add(
          MultipartBody("shopLogo", File(selectedLogo.value?.path ?? "")),
        );
      }
      if (selectedPic.value != null) {
        multipartBody.add(
          MultipartBody("shopPic", File(selectedPic.value?.path ?? "")),
        );
      }
      var response = await apiClient.multipartRequest(
        body: body,
        url: ApiUrl.shopRegistration(),
        reqType: 'POST',
  ...
  petRegistrationUpLoadingMethod(bool status) =>
      petRegistrationUpLoading.value = status;
  ...
  Future<void> petRegistration({required Map<String, String> body}) async {
    try {
      ...
      if (selectedPetPhoto.value != null) {
        multipartBody.add(
          MultipartBody("petPhoto", File(selectedPetPhoto.value?.path ?? "")),
        );
      }
      var response = await apiClient.multipartRequest(
        body: body,
        url: ApiUrl.petRegistration(),
        reqType: 'POST',
        multipartBody: multipartBody,
      );
      if (response.statusCode == 201) {
